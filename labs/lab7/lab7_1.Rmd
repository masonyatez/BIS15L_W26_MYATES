...
msleep %>% 
  filter(bodywt<10) %>% 
  summarize(mean_body_wt_sm=mean(bodywt))
```
...
msleep %>% 
  filter(bodywt > 150) %>% 
  summarize(
    mean_bodywt_lg = mean(bodywt),
    min_bodywt_lg = min(bodywt),
    max_bodywt_lg = max(bodywt),
    sd_bodywy_lg = sd(bodywt),
    total=n()
    )
```
...
msleep %>% 
  ...
  summarize(
    primate_mean_bodywt = mean(bodywt),
    primate_max_bodywt = max(bodywt),
    primate_min_bodywt = min(bodywt),
...
msleep %>% 
  ...
  geom_col() #use this for x and y bar plots
```
