...
https://github.com/masonyatez/BIS15L_W26_MYATES
...
anolis <- anolis %>% 
  mutate(
    across(c(habitat, island), as.factor)
         )
```
...
Cuba, Hispaniola, Jamacia, and Puerto Rico are the islands sampled in the data.
...
Sampling is not equal across the islands. The majority of species sampled come from Cuba and Hispaniola, with Jamaica and Puerto Rico having only a few species sampled from them.
...
anolis %>%
  ...
  labs(
    title= "Amount of Anolis Species Collected from Each Island",
    x = "Island Name",
    y = "Species Count",
    fill = "Island Name"
  )
...
The habitat types: trunk-ground, twig, grass-bush, and trunk-crown are represented in the data.
...
Sampling is not equal across habitat types. The majority of the sampled species come from grass-bush or trunk-ground habitats.
...
anolis %>%
  ...
  group_by(habitat) %>% 
  distinct(species) %>% 
  summarize(habitat_count = n_distinct(species)) %>% 
  ggplot(
    mapping=aes(
      x=habitat,
      y=habitat_count,
  ...
  labs(
    title= "Amount of Anolis Species Collected from Each Habitat",
    x = "Habitat Type",
    y = "Species Count",
    fill = "Habitat Type"
  )
...
Species from trunk-ground habitats have the longest average hindlimb length while species from twig habitats have the lowest average hindlimb length.
...
anolis %>% 
  ...
  labs(
    title = "Hindlimb Lengths (mm) of Species from Varying Habitat Types",
    x = "Habitat Type",
...
anolis <- anolis %>% 
  mutate(
    ratio_of_hindlimb_to_body = signif(hindlimb_length_mm / body_length_mm, digits = 4)
  ) %>% 
  mutate(
    ratio_of_tail_to_body = signif(tail_length_mm / body_length_mm, digits = 4)
  )
...
This plot conveys the information of the ratio of hindlimb to body, while the plot in Problem 10 only presented hindlimb length. the plot from Problem 10 shows that trunk-ground species of anolis after have the longest limbs, while this plot shows that trunk-ground species are the most likely to have similarly sized bodies and hindlimbs.
...
anolis %>% 
  ...
  labs(
    title = "Ratio of Hindlimb to Body Length of Anolis Species Collected from Habitats",
    x = "Habitat Type",
    y = "Hindlimb Length (mm) / Body Length (mm)",
    fill = "Habitat Type"
  )
```
...
This plot suggests that as body length increases, tail length also increases. Species from the grass-bush habitat do approximately follow the same ratio as species taken from other habitats, but have a much higher initial tail length.
...
anolis %>% 
  ...
  scale_color_manual(
    values = habitat_colors_2
  )+
  labs(
    title = "Body Length (mm) vs Tail Length (mm) of Anolis Species",
    x = "Body Length (mm)",
    y = "Tail Length (mm)",
    color = "Habitat Type"
  )
...
The mean number of toepad lamelle, rounded to four significant figures, from the habitats grass-bush, trunk-crown, trunk-ground, and twig are 28.31, 38.50, 29.96, and 27.60 respectively.
...
anolis %>% 
  ...
  group_by(habitat) %>% 
  summarize(
    mean_lamelae_count = signif(mean(toepad_lamellae_count), digits = 4)
  )
```
...
Toepad lamelle count ranges are not consistent across all islands, with Cuba having a much higher average count than any of the other islands.
...
anolis %>% 
  ...
  labs(
    title = "Range of Toepad Lamelle Counts Among Anolis Species Sampled",
    x = "Island Name",
    y = "Toepad Lamelle Count",
    fill = "Island Name"
  )
```
