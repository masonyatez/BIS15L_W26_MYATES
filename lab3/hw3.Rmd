...
```{r}
height_ft_mean <- mean(mountains$height_feet)
```
...
```{r}
mountains <- tibble(name, height_feet, height_meters, first_climbed)
```
...
```{r}
summits <- c(12884, 800, 532, 933, 450)
...
for (i in 1:5){
  addition <- signif(fatalities[i]/summits[i]*100, digits = 3)
  fatality_rate <- c(fatality_rate, addition)
}
